...
INSERT INTO sabor_ecuatoriano.categoria (nombre) VALUES ('Sopas'), ('Platos fuertes'), ('Mariscos'), ('Desayunos'), ('Bebidas'), ('Snacks'), ('Postres');

ALTER TABLE sabor_ecuatoriano.usuario ADD COLUMN IF NOT EXISTS rol_id INTEGER;

UPDATE sabor_ecuatoriano.usuario u SET rol_id = r.id FROM sabor_ecuatoriano.rol r WHERE r.nombre = u.role;
...
ALTER TABLE sabor_ecuatoriano.usuario ADD CONSTRAINT usuario_rol_id_fkey FOREIGN KEY (rol_id) REFERENCES sabor_ecuatoriano.rol(id) ON DELETE RESTRICT ON UPDATE CASCADE;

ALTER TABLE sabor_ecuatoriano.producto ADD COLUMN IF NOT EXISTS categoria_id INTEGER;

UPDATE sabor_ecuatoriano.producto p SET categoria_id = c.id FROM sabor_ecuatoriano.categoria c WHERE c.nombre = p.categoria;

ALTER TABLE sabor_ecuatoriano.producto DROP CONSTRAINT IF EXISTS producto_categoria_id_fkey;
ALTER TABLE sabor_ecuatoriano.producto ADD CONSTRAINT producto_categoria_id_fkey FOREIGN KEY (categoria_id) REFERENCES sabor_ecuatoriano.categoria(id) ON DELETE SET NULL ON UPDATE CASCADE;
